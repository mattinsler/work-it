...
TaskManager.prototype.getQueueStats = function(queueNames) {
  var self = this;
  var stats = {};

  function fetch(queues) {
    return q.all(
  ...
  if (queueNames && Array.isArray(queueNames) && queueNames.length > 0) {
    return fetch(queueNames);
  } else {
    return this.getQueueNames().then(fetch);
  }
};

TaskManager.prototype.getDeepQueueStats = function(queueNames) {
  var self = this;
  var stats = {};
