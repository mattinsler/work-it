...
TaskManager.prototype.queueTask = function(command, data) {
  ...
  var self = this;
  return this.queueProvider.get(command).push(task).then(function() {
    return self.taskTracker(task.id);
  });
};

TaskManager.prototype.taskTracker = function(taskId) {
  return new TaskTracker(taskId, this);
};

TaskManager.prototype.tasksOlderThan = function(command, ts) {
  var queue = this.queueProvider.get(command);
  
  return queue.getPopClient().then(function(client) {
    return q.ninvoke(client, 'zrevrangebyscore', queue.workQueue, ts, '-inf');
  });
};
