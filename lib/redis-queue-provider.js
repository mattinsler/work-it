...
var RedisQueue = function(provider, name, opts) {
  ...
  this.__defineGetter__('client', function() {
    if (!client) {
      client = provider.createConnection();
    }
    return client;
  });
};
...
RedisQueue.prototype.push = function(message) {
  ...
  return q.ninvoke(this.client, 'lpush', this.taskQueue, encoder.encode(message));
};
...
RedisQueue.prototype.complete = function(messageID) {
  ...
  if (!message) { return q(); }
  return q.ninvoke(this.client, 'lrem', this.workQueue, -1, message);
};
...
RedisQueue.prototype._churnPop = function() {
  ...
  q.ninvoke(client, 'rpoplpush', this.taskQueue, this.workQueue).then(function(message) {
    if (message) {
