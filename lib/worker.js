...
Worker.prototype.startTask = function(envelope) {
  ...
  return q().then(function() {
    ...
    task.failedCount = envelope.failedCount || 0;
    task.reapedCount = envelope.reapedCount || 0;
  
    child = self.getChild(task.id, envelope.id);
    ++child.executionCount;
  
    // setup logging, record times, start kill timer, etc.
    self.taskData[task.id] = {
      execution: now,
      startedAt: now,
      stats: []
    };
    ...
    if (self.storageProvider) {
      return self.storageProvider.get(task.id, now).start({
        queue: self.queue.name,
        data: task.data,
        queuedAt: new Date(task.ts),
...
Worker.prototype.finishTask = function(envelope, err) {
  ...
  return q().then(function() {
    var now = Date.now();
    var task = envelope.task;
    var taskData = self.taskData[task.id];
    ...
    if (self.storageProvider) {
      return self.storageProvider.get(task.id, taskData.execution).finish({
        finishedAt: new Date(taskData.finishedAt),
...
Worker.prototype.executeTask = function(envelope) {
  ...
  return this.startTask(envelope).then(function(worker) {
    return worker.execute(envelope.task.data, envelope.task).then(function() {
      return self.finishTask(envelope).then(function() {
      ...
      return self.finishTask(envelope, err).then(function() {
        return self.queue.fail(envelope.id);
      });
    });
  });
};
...
Worker.prototype._cycle = function() {
  ...
  this.queue.pop(this.machineId).then(function(envelope) {
