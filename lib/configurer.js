...
Configurer.prototype.worker = function(commandName, workerFile, opts) {
  ...
  if (this.config.logProviders) {
    var self = this;
    logProvider = workit.Providers.Log.Multi(
      Object.keys(this.config.logProviders).map(function(key) {
        return self.getProvider(workit.Providers.Log, self.config.logProviders[key]);
      })
    );
  } else if (this.config.logProvider) {
    logProvider = this.getProvider(workit.Providers.Log, this.config.logProvider);
  } else {
    logProvider = workit.Providers.Log.Console();
  }
...
Configurer.prototype.generateConfig = function() {
  ...
  if (this.config.monitoringProvider) {
    opts.monitoring = this.getProvider(workit.Providers.Monitoring, this.config.monitoringProvider);
  }
  ...
  opts.heartbeat = this.getProvider(workit.Providers.Heartbeat, this.config.statusProvider.eventsProvider);
  
  return opts;
};

Configurer.prototype.getProvider = function(providers, configValue, opts) {
  ...
  return providerType(value, opts);
};
